        ...
        + 'static,
{
    ...
    fn publish_pending_blocks(
        ...
        let pending_blocks = Arc::new(pending_blocks_builder.build(Some(received_at))?);
    ...
    fn process_canonical_block(
        ...
        match strategy {
            ...
            ReconciliationStrategy::HandleReorg => {
                warn!(
                    message =
                        "reorg detected, recomputing pending flashblocks going ahead of reorg",
    ...
    fn build_pending_state_for_same_block(
        ...
        self.publish_pending_blocks(pending_blocks_builder, db, state_overrides, received_at)
    }
    ...
    fn build_pending_state(
        ...
        // Carry forward the historical override/bundle snapshots for pending-state serving, then
        // publish (which merges transitions, captures the bundle state, builds, and retains the
        // live db for incremental reuse by the next flashblock).
