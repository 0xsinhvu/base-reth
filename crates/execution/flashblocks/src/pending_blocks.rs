...
use alloy_primitives::{
    Address, B256, BlockNumber, TxHash, U256, map::foldhash::HashMap as FbHashMap,
};
...
impl PendingBlocksBuilder {
    ...
    pub fn from_previous(pending_blocks: &PendingBlocks) -> Self {
        ...
        let historical_state_overrides = pending_blocks.historical_state_overrides.clone();
        let historical_bundle_states = pending_blocks.historical_bundle_states.clone();
        ...
        Self {
            ...
            transaction_senders,
            state_overrides,
            historical_state_overrides,
            historical_bundle_states,
            transaction_results,
            execution_times,
            state_root_times,
    ...
    pub fn build(mut self, received_at: Option<u64>) -> Result<PendingBlocks, StateProcessorError> {
        ...
        let bundle_state = self.bundle_state.take().unwrap_or_default();
...
impl PendingBlocks {
    ...
    pub fn get_flashblock_with_logs(&self) -> FlashblockWithLogs {
        ...
        let mut logs = FbHashMap::default();
...
mod tests {
    ...
    fn from_previous_preserves_bundle_state() {
        ...
        let pending_blocks = builder.build(None).expect("build should succeed");
        let next_builder = PendingBlocksBuilder::from_previous(&pending_blocks);
        let next_pending_blocks =
            next_builder.build(None).expect("build from previous should succeed");
    ...
    fn from_previous_preserves_next_position_per_block() {
        ...
        let next_pending_blocks =
            next_builder.build(None).expect("build from previous should preserve positions");
    ...
    fn build_pending_blocks_with_logs(entries: &[(B256, Address)]) -> PendingBlocks {
        let header = Sealed::new_unchecked(Header { number: 1, ..Header::default() }, B256::ZERO);
    ...
    fn build_rejects_duplicate_transaction() {
        ...
        let err = builder.build(None).expect_err("build should fail on duplicate tx");
    ...
    fn build_fallbacks_use_latest_block_values_only() {
        ...
        let pending_blocks =
            builder.build(None).expect("build should succeed without latest context");
